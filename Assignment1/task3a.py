...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO implement this function (Task 3a)
        A =np.exp(X.dot(self.w)) #makes wT*x
        
        #We have to put axis=1 to represent k classes
        B = np.sum(np.exp(X.dot(self.w)), axis=1)[:, None]  
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        self.grad = (-(targets - outputs).T.dot(X)).T / batch_size
...
def one_hot_encode(Y: np.ndarray, num_classes: int):
    ...
    one_hot_encod = np.zeros((Y.shape[0], num_classes), dtype=int)
    aux=Y.flatten()
    # In index corresponding to label, insert 1
    one_hot_encod[np.array(range(len(Y))), aux] = 1
