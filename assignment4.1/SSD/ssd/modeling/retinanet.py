...
class RetinaNet(nn.Module):
    ...
    def __init__(self,
        ...
        num_channels=256 #as in the paper
        # 4 conv layers followed by Relu activation
        for n_boxes, out_ch in zip(anchors.num_boxes_per_fmap, self.feature_extractor.out_channels):
            self.regression_heads.append(nn.Sequential(
                ...
                nn.Conv2d(in_channels=num_channels,out_channels=num_channels,kernel_size=3, stride=1,padding=1),
                nn.ReLU(),
                nn.Conv2d(in_channels=num_channels,out_channels=n_boxes*4,kernel_size=3, stride=1,padding=1)
            ...
            self.classification_heads.append(nn.Sequential(
                ...
                nn.Conv2d(in_channels=num_channels,out_channels=num_channels,kernel_size=3, stride=1,padding=1),
                nn.ReLU(),
                nn.Conv2d(in_channels=num_channels,out_channels=n_boxes*num_classes,kernel_size=3, stride=1,padding=1)
        ...
        self._init_weights(boolea)

    def _init_weights(self,initialization:bool):
        
        if(initialization): #for 2_3_4
            p=0.99
            K=self.num_classes
            b=log(p*(K-1)/(1-p))
            ...
            self.regression_heads[:-1].bias.data.fill_(0) 
            self.classification_heads[:-1].bias.data.fill_(0) 
            self.classification_heads[-1].bias[:6].data.fill_(b) 
